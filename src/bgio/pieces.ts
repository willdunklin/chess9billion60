...
export class Piece {
    ...
    getAttackedSquares(_x: number, _y: number, _gameboard: Array<Array<string|null>>|null, _color: string, display_all?: boolean): number[][] {
        return [];
    }
    ...
    constructor(id: string, strength: number, atoms: Array<[string, number]>,
                move_function?: (x: number, y: number, gameboard: Array<Array<string | null>> | null, color: string, display_all?: boolean) => number[][],
                attack_function?: (x: number, y: number, gameboard: Array<Array<string | null>> | null, color: string) => number[][]) {
        ...
        if (attack_function !== undefined)
            this.getAttackedSquares = (x,y,gameboard,color) => this.getAvailableMoves(x,y,gameboard,color).concat(attack_function(x,y,gameboard,color));
        else
            this.getAttackedSquares = this.getAvailableMoves;
...
const il_vaticano = (attack: boolean = false) => {
    const fun = (x: number, y: number, history: Array<Array<string | null>> | null, color: string, display_all?: boolean) => {
        let moves: number[][] = [];
        if (history === null)
            return moves;
...
const B   =  new Piece("B"  , 315   , [["F", 0]], il_vaticano(), il_vaticano(true)).name("Bishop").setBlurb("A chess classic").setRules("Standard bishop moves").markColorbound();
...
const F   =  new Piece("F"  , 150   , [["F", 1]], il_vaticano(), il_vaticano(true)).name("Ferz").setBlurb("Slow and steady wins the race").setRules("Bishop moves up to one square").markColorbound();
...
const AB  =  new Piece("AB" , 770   , [["F", 0], ["N", 1]], il_vaticano(), il_vaticano(true)).name("Archbishop").setBlurb("Owns a cool cathedral").setRules("Combo bishop and knight").allowOnlyOne();
...
const B4  =  new Piece("B4" , 250   , [["F", 4]], il_vaticano(), il_vaticano(true)).name("Short Bishop").setBlurb("Not as good as a bishop").setRules("Bishop moves up to four squares").markColorbound();
const B2  =  new Piece("B2" , 220   , [["F", 2]], il_vaticano(), il_vaticano(true)).name("Shorter Bishop").setBlurb("Not as good as a short bishop").setRules("Bishop moves up to two squares").markColorbound();
const U   =  new Piece("U"  , 900   , [["N", 0], ["F", 0]], il_vaticano(), il_vaticano(true)).name("Unicorn").setBlurb("Twelve directions!").setRules("Combo knightrider and bishop").allowOnlyOne();
...
const BC  =  new Piece("BC" , 750   , [["F", 0], ["C", 1]], il_vaticano(), il_vaticano(true)).name("Bishop Camel").setBlurb("Queen tier, but just on one color!").setRules("Combo Camel (3,1) and Bishop").markColorbound();
...
const BM  =  new Piece("BM" , 550   , [["W", 1], ["F", 1], ["F", 0]], il_vaticano(), il_vaticano(true)).name("Cardinal").setBlurb("Can reach both colors!").setRules("Combo Bishop and King");
